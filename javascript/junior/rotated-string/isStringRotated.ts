...
console.log(
  `1 way: with for loop. "${stringSource}" - "${string1For}" ${output1For}`
);
console.log(
  `1 way: with for loop. "${stringSource}" - "${string2For}" ${output2For}`
);
console.log(
  `1 way: with for loop. "${stringSource}" - "${string3For}" ${output3For}`
);

/* 2 way: with concatenation and string length check */
const isStringRotatedDoubleLength = (
  inputString1: string,
  inputString2: string
): boolean => {
  return (
...
console.log(
  `2 way: string length check, "${stringSource}" - "${string1Double}" ${output1Double}`
);
console.log(
  `2 way: string length check, "${stringSource}" - "${string2Double}" ${output2Double}`
);
console.log(
  `2 way: string length check, "${stringSource}" - "${string3Double}" ${output3Double}`
);
